...
const EditShippingProductInformation = ({ product, onData }) => {
    ...
    useEffect(() => {
        if (product) {
            const setProductData = async () => {
                try {
                    console.log('Hellooooooooooooooooooooooo:', product.ProductVarients[0].length);
                    dispatch({ type: 'set_length', value: product.ProductVarients[0].length });
                    dispatch({ type: 'set_width', value: product.ProductVarients[0].width });
                    dispatch({ type: 'set_height', value: product.ProductVarients[0].height });
                    dispatch({ type: 'set_weight', value: product.ProductVarients[0].weight });
                    form.setFieldsValue({
                        length: product.ProductVarients[0].length,
                        width: product.ProductVarients[0].width,
                        height: product.ProductVarients[0].height,
                        weight: product.ProductVarients[0].weight
                    });
                } catch (error) {
...
export default EditShippingProductInformation