...
import { sendEmailVerificationAgainForAdmin } from "../../firebase/AuthenticationFirebase";
...
const UpdateOTP = () => {
  ...
  const handleOnSendVerifyEmail = async () => {
    ...
    try {
      await sendEmailVerificationAgainForAdmin(localState.password);
      setLocalState({
        type: "success",
  ...
  const handleRegisterNewOTP = async () => {
    ...
    try {
      ...
      if (res.success) {
        ...
        navigate("/admin/account?type=security-password&step=3");
      }
    } catch (error) {
