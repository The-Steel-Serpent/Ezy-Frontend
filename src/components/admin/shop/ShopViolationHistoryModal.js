...
const ShopViolationHistoryModal = ({ visible, onClose, userId }) => {
  ...
  return (
    <Modal
      title="Lịch sử xử lý vi phạm của cửa hàng"
      visible={visible}
      onCancel={onClose}
      footer={null}
    >
        ...
        renderItem={(history) => (
          <List.Item key={history.violation_history_id}>
            <List.Item.Meta
              ...
              description={`Loại xử lý: ${history.action_type} | Hành động: ${history.action_type} | Trạng thái: ${history.status} | Ghi chú: ${history.notes || "Không có"} | Người cập nhật: ${history.updated_by_id}`}
            />
          </List.Item>
        )}
      />
